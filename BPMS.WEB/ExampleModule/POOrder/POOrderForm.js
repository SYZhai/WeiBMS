/// <reference path="../../CommonModule/Employee/EmployeeList.aspx" />
/// <reference path="../../CommonModule/Employee/EmployeeList.aspx" />
/// <reference path="MyPOOrder_List.aspx" />
//初始化
...
function SalesmanAutocomplete() {
    ...
    function LoadManager(parm) {
        getAjax('../../CommonModule/Employee/EmployeeList.aspx', parm, function (data) {
            var json = eval("(" + data + ")");
            var html = "";
...
function add_ColumnName(key) {
    ...
    if (key != "") {
        ...
        getAjax('POOrder_Form.aspx', parm, function (data) {
            var json = eval("(" + data + ")");
            for (var i = 0; i < json.JSON.length; i++) {
                var list = json.JSON[i];
...
function GoodsAutocomplete(obj) {
    $("#BarCode➩" + obj).bind("keyup", function (e) {
        ...
        if (e.which != 13 && e.which != 40 && e.which != 38) {
            ...
            getAjax('POOrderForm.aspx', parm, function (data) {
                var json = eval("(" + data + ")");
                var html = "";
                for (var i = 0; i < json.JSON.length; i++) {
                    var list = json.JSON[i];
...
function Commit_Save() {
    ...
    top.showConfirmMsg('注：您确认要保存此操作吗？', function (r) {
        if (r) {
            ...
            getAjax('POOrder_Form.aspx', parm, function (rs) {
                if (parseInt(rs) > 0) {
                    Replace();
